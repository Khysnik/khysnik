...
function updatelog($resno=0){
	...
	for($page=0;$page<$counttree;$page+=PAGE_DEF){
		...
		for($i = $st; $i < $st+PAGE_DEF; $i++){
			...
			//  Main creation
			//$dat.="bla <input type=\"checkbox\" name=\"$no\" value=\"delete\" /> <span class=\"filetitle\">$sub</span> ";
			$dat.=" 🔶︎ <span class=\"filetitle\">$sub</span> ";
			...
			while($resrow=mysql_fetch_row($resline)){
				...
				//$dat.="bla <span class='intro'><input type=\"checkbox\" name=\"$no\" value=\"delete\" /> $replytitle";
				$dat.="<span class='intro'> 🔶︎ $replytitle";
...
<td><input type="file" name="upfile" size="35" />';
	...
	$dat.='<input type="submit" value="'.S_SUBMIT.'" /></td></tr>
<tr><td colspan="2">
