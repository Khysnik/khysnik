...
const S_OEKAKILOAD = 'Click here to load oekaki.';
...
const S_RULES = '<h3 style="text-align:center;color:#CC1105;">NSFW content is not permitted [whole website permaban].</h3><ul><li>Supported file types are: GIF, JPG. PNG</li>
<li>Paste url to embed yt video or soundcloud tracks</li><li>Images greater than '.MAX_W.'x'.MAX_H.' pixels will be thumbnailed.</li><li>The last thread created removes the oldest thread so do not multiply threads for nothing. Please use REPLY when your post is linked to an existing thread.</li></ul>';				//Prints rules under posting section
const S_RULES_SWF = '<h3 style="text-align:center;color:#CC1105;">NSFW content is not permitted [whole website permaban].</h3><ul><li>Supported file types are: GIF, JPG. PNG, SWF</li>
<li>Paste url to embed yt video or soundcloud tracks</li><li>Images greater than '.MAX_W.'x'.MAX_H.' pixels will be thumbnailed.</li><li>The last thread created removes the oldest thread so do not multiply threads for nothing. Please use REPLY when your post is linked to an existing thread.</li></ul>';				//Prints rules under posting section
const S_RULES_WEBM = '<h3 style="text-align:center;color:#CC1105;">NSFW content is not permitted [whole website permaban].</h3><ul><li>Supported file types are: GIF, JPG. PNG, WEBM</li>
<li>Paste url to embed yt video or soundcloud tracks</li><li>Images greater than '.MAX_W.'x'.MAX_H.' pixels will be thumbnailed.</li><li>The last thread created removes the oldest thread so do not multiply threads for nothing. Please use REPLY when your post is linked to an existing thread.</li></ul>';				//Prints rules under posting section
const S_RULES_BOTH = '<h3 style="text-align:center;color:#CC1105;">NSFW content is not permitted [whole website permaban].</h3><ul><li>Supported file types are: GIF, JPG. PNG, SWF, WEBM</li><li>Paste url to embed yt video or soundcloud tracks</li><li>The last thread created removes the oldest thread so do not multiply threads for nothing. Please use REPLY when your post is linked to an existing thread.</li></ul>';	// <li>Images greater than '.MAX_W.'x'.MAX_H.' pixels will be thumbnailed.</li>
...
const S_FOOT = ''; //Prints footer (leave these credits)
...
const S_ANONAME = 'Anonymoose';										//Defines what to print if there is no text entered in the name field
...
const S_MANAREPDEL = 'Delete Panel';									//Defines Management Panel button--allows the user to view the management panel (overview of all posts)
const S_MANABAN = 'Ban Panel';										//Defines Manager Ban button
const S_MANAPOST = 'Mod Post';									//Defines Manager Post button--allows the user to post using HTML code in the comment box
const S_MANAACCS = 'Mod Panel';								//Defines button for adding/removing manager accoutns
...
const S_MDTABLE1 = '<th>Offline</th><th>Delete?</th><th>Post No.</th><th>Time</th><th>Subject</th>';			//Explains field names for Management Panel (Delete?->Subject)
const S_MDTABLE2 = '<th>Name</th><th>IP</th><th>Comment</th><th>Size<br />(Bytes)</th><th>md5</th><th>Reply #</th><th>Timestamp (s)</th><th>Timestamp (ms)</th>';	//Explains names for Management Panel (Name->md5)
...
const S_FILTER1WORD = 'Error: It seems that your post is not interesting.';								
const S_FILTERWIIU = 'Error: Enough WiiU screenshots.';								
const S_FILTERSMALL = 'Error: It seems that your image is lame.';								
