...
//browser only?
io.set('origins', 'www.windows93.net:8089');
  ...
  socket.on('disconnect', function () {
    ...
    socket.on('magnet index', function(data) {
      ...
      for (var i = 0; i < magnets.length; i++) {
        ...
        /*
        if (userPengGiven[address]!=undefined) {
          magnets[i].pengs = userPengs[id].users.length-userOofs[id].users.length;
        };
        */


        // userPengGiven[address].pengs.indexOf(id);

        /*
        score=0;
        if (userPengs[id]!=undefined) {score=score+userPengs[id].users.length};
        if (userOofs[id].users!=undefined) {score=score-userOofs[id].users.length};

        //if(pengs[id]!=undefined){
         magnets[i].pengs = userPengs[id].users.length-userOofs[id].users.length; 
         console.log('userPengs: '+userPengs[id].users.length);
         console.log('userOofs: '+userOofs[id].users.length);
