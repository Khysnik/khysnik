...
?>
  <script>
    //if (location.protocol == 'https:'){ window.location='http://windows93.net/trollbox/' }
  </script>
...
<div id="trollbox" class="skin_base ui_layout _ui_unselectable">
  ...
  <script>   
    ...
    var helpMsg ="\n"+
      ...
      helpMsg=helpMsg+"| /zalgo text            zalgo text generator                                       |\n"+    
      "| /ascii imageUrl        ascii art converter                                        |\n"+
      "| /who                   list online users                                          |\n"+
      "| /block username        block user (or right click username)                       |\n"+
      "| /unblock username      unblock user (or click username)                           |\n"+
  ...
  <script src="//<?= $_SERVER['SERVER_NAME'] ?>:<?php echo $port;?>/socket.io/socket.io.js"></script>
  ...
  <script>
    ...
    var socket = io('//<?= $_SERVER['SERVER_NAME'] ?>:<?php echo $port;?>');
    ...
    function chatKing(){
      ...
      $("#trollbox_infos div span").first().before("👑 ");
      
      $( "#trollbox_infos div" ).contextmenu(function() {
        sendMsg('/block '+$(this).children('span').text())
        return false
      });
      $( "#trollbox_infos div" ).click(function() {
        sendMsg('/unblock '+$(this).children('span').text())
        return false
      });   
    }
  ...
  function escapeRegExp(str) {
      ...
      if (typeof msg === 'string') {
        if (cmd) {
          ...
          if (cmd.cmd === 'pitch') {
            ...
            dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: "Pitch set to: "+pitch };
            printMsg(dada);
            return;
          }     
          ...
          if (cmd.cmd === 'rate') {
            ...
            dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: "Rate set to: "+pitch };
            printMsg(dada);
            return;
          }    
          ...
          if (cmd.cmd === 'voice'&&voices.length>0) {
            ...
            dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: "Voice set to: "+voices[voice].name };
            printMsg(dada);
            return;
          } 
          ...
          */      
          ...
          if (cmd.cmd === 'emo') {
            if (cmd.val=='on') {
              ...
              dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: "Emoticon: ON" };
              printMsg(dada);
            }
            if (cmd.val=='off') {
              emoticons=false;
               $store.set('.config/trollbox/emoticons', emoticons);
              dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: "Emoticon: OFF" };
              printMsg(dada);
            }
            return;
          }
          if (cmd.cmd === 'sin') {
            ...
            if (cmd.val=='off') {
              sin=false;
               $store.set('.config/trollbox/sin', sin);
              dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: "Sin: OFF" };
              printMsg(dada);
              return;
            }
          }
          ...
          if (cmd.cmd === 'mlg') {
            if (cmd.val=='on') {
              ...
              emoticons=true;
               ...
               $store.set('.config/trollbox/speech', speech);
               $store.set('.config/trollbox/emoticons', emoticons);
              dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: "Major League Gaming: ON" };
              printMsg(dada);
            }
            if (cmd.val=='off') {
              imgShow=false;
              ...
              emoticons=false;
               ...
               $store.set('.config/trollbox/speech', speech);
               $store.set('.config/trollbox/emoticons', emoticons);
              dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: "Major League Gaming: OFF" };
              printMsg(dada);
            }
            return;
          }  
          ...
          if (msg.startsWith('/font ')) {
            ...
            if (fontNames[(myfont%fontNames.length)]) {
              ...
              dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: temp }; 
              printMsg(dada);
              return;             
            }
          };
        ...
        if (msg=='/scroll') { 
          dada = { date: Date.now(), nick: "~", color: "white", style: "opacity: 0.7;", msg: 'Auto Scroll: '+!scroll };
          scrollDown ()
          scroll=!scroll;
          printMsg(dada);
          return;
    ...
    socket.on('user change nick', function (old, nu) {
        ...
        for (var i = 0; i < blocked.length; i++) { if (nu.home==blocked[i]) {return} };
  ...
  <script>
    ...
    function dogescii(url){
      ...
      if (test) { 
        nik=pseudo;
        if (nik!=" ") {
          url = 'https://www.windows93.net/trollbox/pix.php?url='+url+'&nick='+pseudo;
      ...
      /*
      dada = { date: Date.now(), nick: "Ascii", color: "white", style: "opacity: 0.7;", msg: "Disabled, sorry." };
      printMsg(dada);
      */
    }
