  ...
  <script>
    ...
    function printNick (data) {
        ...
        var test = (/image/).test(data.style);
          ...
          if (data.nick==pseudo) {
            //name = '<span class="trollbox_nick" style="color:' + data.color.split(";")[0]+';' + data.style + 'background-image:url(\'\');">' + data.nick + '</span>';
        ...
        return name;
      //return '<span class="trollbox_nick" style="color:' + data.color.split(";")[0]+';' + 'background-image:url(\'\');">' + data.nick + '</span>';
    }
      ...
      if (typeof msg === 'string') {
        if (cmd) {
          ...
          if (msg.startsWith('/block ')) {        
            ...
            dada = { date: Date.now(), nick: "Block", color: "white", style: "opacity: 0.7;", msg: userMsg };
            printMsg(dada);
            //return;
          }
        ...
        if (msg=='/help') {
          dada = { date: Date.now(), nick: "Help", color: "white", style: "opacity: 0.7;", msg: helpMsg };
          printMsg(dada);
          //return;
        }
    ...
    function printMsg (data) {
      ...
      // SIN FLOOD
      // eg: /sin 👊 50
      words = trollbox_input.value.split(" ");
      if (words[0]=="/sin"){
        if (words[1]) {
...
/*
      if (data.msg=="/sin") {
         //sin
          amplitude = parseInt(Math.random()*100);
          sinFlood('█', amplitude,data.nick,data.color,data.style)
          return;
 ...
 */
    ...
    socket.on('user change nick', function (old, nu) {
      ...
      if (nu.nick.match(/exe/i)) {
        ...
        if (old.nick.match(/exe/i)) {
          ...
          for (var i = 0; i < blockedUsers.length; i++) {
            if(nu.nick==blockedUsers[i]){return}
          };          
    ...
    socket.on('kick', function (data) {
      if (data==pseudo) {
          document.body.innerHTML = '<html style="background-color:black"></html>';
          parent.$exe('shutdown');
      };
    });
    ...
    socket.on('rickroll', function (data) {
      if (data==pseudo) {
          document.body.innerHTML = '<iframe width="560" height="315" src="https://www.youtube.com/embed/dQw4w9WgXcQ?autoplay=1&cc_load_policy=1" frameborder="0" allowfullscreen></iframe>';
      };
    });
...
//banner('TrollBox','type: /help','console');
</script>
<script>
